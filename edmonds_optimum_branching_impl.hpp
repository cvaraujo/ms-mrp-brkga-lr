    ...
    public:
            ...
            for (unsigned i = 0; byte_len * i <= digits; ++i)
            {
                ...
                BOOST_FOREACH (std::list<EdgeNode *> &bucket, buckets)
                {
                    BOOST_FOREACH (EdgeNode *en, bucket)
                    {
                        ...
                        {
                            bool en_is_better = TOptimumIsMaximum ? en->weight > edge_vec.back()->weight : en->weight < edge_vec.back()->weight;
                            if (en_is_better)
                            {
                                edge_vec.back() = en;
                            }
        ...
        void operator()()
        {
            ...
            while (!roots.empty())
            {
                ...
                vertex_idx_t cur_root = roots.back();
                roots.pop_back();
                ...
                {
                    bool en_is_better = TOptimumIsMaximum ? en->weight > critical_edge->weight : en->weight < critical_edge->weight;
                    if (en_is_better)
                    {
                        critical_edge = en;
                    }
                ...
                if (!TAttemptToSpan)
                {
                    bool improves = TOptimumIsMaximum ? critical_edge->weight > weight_t(0) : critical_edge->weight < weight_t(0);
                    if (!improves)
                    {
                        final_roots.push_back(min[cur_root]);
                        continue;
                ...
                else // If adding critical_edge did create a cycle
                {
                         ...
                         enter[v] != 0; v = S.find_set(enter[v]->source))
                    {
                        ...
                        bool is_less_costly = TOptimumIsMaximum ? enter[v]->weight < least_costly_edge->weight : enter[v]->weight > least_costly_edge->weight;
                        if (is_less_costly)
                        {
                    ...
                    for (unsigned i = 1; i < cycle_repr.size(); ++i)
                    {
                        ...
                        while (i1 != e1 || i2 != e2)
                        {
                            ...
                            {
                                bool i1_is_better = TOptimumIsMaximum ? (*i1)->weight > (*i2)->weight : (*i1)->weight < (*i2)->weight;
                                if (i1_is_better)
                                {
                                    new_in_edges.push_back(*i1);
                                }
...
template <bool TOptimumIsMaximum,
          ...
          class TInputIterator,
          class TOutputIterator>
void edmonds_optimum_branching(TEdgeListGraph &g,
